...
// Error-State EKF (ES-EKF)의 핵심: 모든 상태를 직접 수정하기보다, 
// 오차 dx를 먼저 계산하고 그 오차를 명목 상태(Nominal State)에 반영하는 방식
...
EKF::EKF() {
    ...
    // Measurement Noise(측정 노이즈 행렬, R), 센서가 내주는 값을 얼마나 신뢰하는지 결정
    R_gps_ = MatrixXd::Identity(3, 3) * 0.25; // GPS 오차 약 0.5m -> 공분산 행렬에 들어갈 값은 제곱인 0.25
...
void EKF::update_gps(const Vector3d& meas_pos) {
    ...
    // 내가 예측한 값(P)를 더 믿을지, 아니면 GPS 측정값(R)을 더 믿을지 결정하는 가중치 K 계산
