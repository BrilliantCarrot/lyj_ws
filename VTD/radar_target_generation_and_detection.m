...
% 레이더 신호에서 거리(FMCW 레이더의 경우)를 추정하기 위해 1차 FFT(빠른 푸리에 변환)를 실행하고,
% 이를 통해 목표물까지의 거리를 계산
...
% 절댓값을 취하면 주파수 성분의 크기를 구할 수 있다. 주파수 도메인에서 신호 강도를 계산
sig_fft = abs(sig_fft);

% Output of FFT is double sided signal, but we are interested in only
% one side of the spectrum. Hence we throw out half of the samples(신호 결과는 대칭이기에).
...
% Plot FFT output 
% 신호가 가장 크게 나타나는 값을 기반으로 타겟까지의 거리를 시각화
plot(sig_fft); 
axis([0, 200, 0, 1]);
ylim([0, 0.5])
grid minor;
