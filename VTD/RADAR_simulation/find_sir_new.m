function sig = find_sir_new(radar_pos, target_pos, RADAR,X,Y,Z)
    % radar_pos: 레이더 위치 좌표
    % target_pos: 목표물(각각의 지형)좌표, 상위의 함수에서 particle의 위치
    % RADAR: 레이더 구조체
    % 레이더 파라미터 설정
    % X, Y, Z: 지형 데이터
    ...
    L = 8.17;  % [dB] Loss
    sigma_0 = 10^(-5/10);  % Clutter Scattering Coefficient, 분자값이 감마 계수
