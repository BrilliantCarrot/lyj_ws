...
function [optimal_path, sir_values, visibility_values] = PSO_visibility(radars, start_pos, end_pos, X, Y, Z, RADAR, visibility_matrix)
    ...
    % X, Y, Z: 지형 데이터
    % RADAR: 레이더 구조체
    num_particles = 200;   % 입자 수 줄임 (더 작은 탐색 영역)
    max_iter = 10;         % 반복 수
    w = 0.7;               % 관성 계수
    c1 = 1.5;              % 개인 가속 계수
    c2 = 1.5;              % 전역 가속 계수
    ...
    lookahead_depth  = 2; % 2 스텝까지 미래 비용을 예측
    lookahead_weight = 0.2; % 룩어헤드 누적 비용 가중치
    total_path_length = norm(end_pos - start_pos);
    % sir_data = {};
    search_radius = 500;
    ...
    while norm(current_point - end_pos) > min_distance
        ...
        % current_point 기준으로 가장 가까운 레이더 하나 선택(비용함수에서 가장 가까운 레이더와 멀어지게 하기위함)
        radar_distances = vecnorm(radars - current_point, 2, 2);  
        [~, min_idx] = min(radar_distances);
        active_radar = radars(min_idx, :);
        % 초기 개인 최적값 및 전역 최적값 설정
        pbest = particles;
        pbest_scores = arrayfun(@(i) calculate_fitness(radars, active_radar, total_path_length, ...
            particles(i, :), end_pos, start_pos, visibility_matrix, RADAR, X, Y, Z, ...
            lookahead_depth, lookahead_weight, search_radius), 1:num_particles)';
        ...
        for iter = 1:max_iter
            ...
            for i = 1:num_particles
                ...
                % 현재 입자의 SIR 계산
                current_score = calculate_fitness(radars, active_radar, total_path_length, particles(i, :), ...
                    end_pos, start_pos, visibility_matrix, RADAR, X, Y, Z, lookahead_depth, lookahead_weight, search_radius);
                % 개인 최적값 업데이트
...
function fitness = calculate_fitness(radars, active_radar, total_path_length, particle_pos, end_pos, start_pos, ...
...
function cost = compute_lookahead_cost(radars, cur_pos, end_pos, start_pos, visibility_matrix, RADAR, X, Y, Z, depth, lookahead_weight, search_radius)
    if depth == 0
        cost = 0;
        return
    end
    ...
    for k = 1:numel(angles)
        ...
        total_path_length = norm(end_pos - start_pos);
        scale = total_path_length / 6;
        distance_to_goal = norm(cur_pos - end_pos);
        weight = exp(-distance_to_goal / scale);
        if visibility_matrix(iy, ix) == 0 % 가려진 영역이면 보상 부여
            visibility_bonus = -10 * weight;
            % visibility_bonus = 0;
...
%         % 초기 개인 최적값 및 전역 최적값 설정
%         pbest = particles;
%         pbest_scores = arrayfun(@(i) calculate_fitness(radars, particles(i, :), ...
% end_pos, visibility_matrix, RADAR, X, Y, Z), 1:num_particles)';
%         [gbest_score, gbest_idx] = min(pbest_scores);
%         gbest = pbest(gbest_idx, :);
%         % PSO 반복문
%         % 반복문을 돌며 개별 입자 전체의 SIR 계산
...
%     end
% 
%     fitness = sir_value + 0.001 * distance_to_goal + visibility_bonus;
% end